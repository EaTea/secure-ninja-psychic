...
public class InsecureDeveloper {
	...
	protected File linkFiles(List<File> classFiles,
	        List<License> requestedLicenses, final String jarName,
	        Socket connection) {
			...
			try {
				...
				for (int i = 0; i < nLicReturned; i++) {
					...
					addLicense(libraryName,
					        new License(license, connection.getInetAddress(),
		...
		do {
			...
			if (command.equalsIgnoreCase("Request")) {
				...
				if (requestedLicenses == null) {
					System.out.println("Could not find licenses for "
					        + "requested libraries");
					System.out.println("Will not attempt to link");
					continue;
				}
				...
				if (nFiles <= 0) {
					System.out.printf("%d class files is illegal --- must have"
					        + " at least 1 of your own files\n", nFiles);
					System.out.println("Will not attempt to link");
					continue;
				}
				...
				try {
					Socket connection = new Socket(remoteHost, remotePort);
					linkFiles(classFiles, requestedLicenses, jarFileName,
					        connection);
					connection.close();
				} catch (UnknownHostException e) {
					System.err.println("Error: host name could"
